...
# 在线获取最新脚本并执行
SCRIPT_URL="https://raw.githubusercontent.com/onebokc/tools/main/ssh_tool.sh"
...
# 运行统计
sum_run_times() {
  local COUNT=$(curl -s -m 2 "https://count.eooce.dpdns.org/?url=https://raw.githubusercontent.com/onebokc/tools/main/ssh_tool.sh") &&
...
echo -e "\033[0;97m脚本地址: https://github.com/onebokc/tools\033[0m"
echo ""
echo -e "${skyblue} ##  ## #####   ####       ######  ####   ####  ##      ${re}"
echo -e "${skyblue} ##  ## ##  ## ##            ##   ##  ## ##  ## ##      ${re}"
echo -e "${skyblue} ##  ## #####   ####.        ##   ##  ## ##  ## ##      ${re}"
...
echo "-------------------------------------------------------------------"
echo -e "${green} 1. 系统更新                   2. BBR管理 ${re}"
echo -e "${green} 3. WARP管理 ▶解锁ChatGPT/Netflix${re}"
echo -e "${green} 4. 系统工具 ▶                5. 节点搭建合集 ▶${re}"
echo "-------------------------------------------------------------------"
...
case "$choice" in
  1) choice=2 ;;   # 系统更新(原2)
  2) choice=5 ;;   # BBR管理(原5)
  3) choice=7 ;;   # WARP管理(原7)
  4) choice=10 ;;  # 系统工具(原10)
  5) choice=12 ;;  # 节点搭建合集(原12)
  0) choice=88 ;;  # 退出
  *) echo -e "${red}无效选择，仅支持: 1,2,3,4,5,0${re}"; sleep 1; continue ;;
esac
...
EOF
          ...
          22)
            ...
            while true; do
                ...
                case $sub_choice in
                    1)
                        curl https://raw.githubusercontent.com/onebokc/tools/main/check_cpu.sh -o check_cpu.sh && chmod +x check_cpu.sh && bash check_cpu.sh
                    ...
                    ;;
                    2)
                        curl https://raw.githubusercontent.com/onebokc/tools/main/check_memory.sh -o check_memory.sh && chmod +x check_memory.sh && bash check_memory.sh
                    ...
                    ;;
                    3)
                        curl https://raw.githubusercontent.com/onebokc/tools/main/check_traffic.sh -o check_traffic.sh && chmod +x check_traffic.sh && bash check_traffic.sh
                    ...
                    ;;
                    4)
                        curl https://raw.githubusercontent.com/onebokc/tools/main/check.sh -o check.sh && chmod +x check.sh && bash check.sh
  ...
  12)
    while true; do
      ...
      echo -e "${green}---------------------------------------------------------${re}"
      echo -e "${white} 1. F佬Sing-box一键脚本        3. 老王xray-2go一键脚本${re}"
      echo -e "${white} 2. 老王Sing-box四合一         4. F佬ArgoX一键脚本${re}"
      echo -e "${white} 5. 233boy.sing-box一键脚本    6. Suoha一键Argo脚本${re}"
      echo -e "${yellow}---------------------------------------------------------${re}"
      echo -e "${yellow}        单协议                    XRAY面板及其他${re}"
      echo -e "${yellow}---------------------------------------------------------${re}"
      echo -e "${white} 7. Hysteria2脚本合集         11. 新版X-UI面板一键脚本${re}"
      echo -e "${white} 8. Juicity一键脚本           12. 伊朗版3X-UI面板一键脚本${re}"
      echo -e "${white} 9. 老王Tuic-v5一键脚本       14. sing-box面板(sui) ▶${re}"
      echo -e "${white}10. Snell一键安装脚本         13. 老王Reality一键脚本${re}"
      echo "---------------------------------------------------------"
      echo -e "${skyblue} 0. 返回主菜单${re}"
      echo "---------------"
      read -p $'\033[1;91m请输入你的选择: \033[0m' sub_choice
      ...
      case $sub_choice in
        ...
        4)
        clear
            bash <(curl -Ls https://raw.githubusercontent.com/fscarmen/ArgoX/main/argo.sh)
            break_end
        ;;
        5)
        clear
  ...
  00)
    cd ~
    curl -sS -O https://raw.githubusercontent.com/onebokc/tools/main/update_log.sh && chmod +x update_log.sh && ./update_log.sh
    rm update_log.sh
    echo ""
